...
// Remove old admin and create new one
db.prepare("DELETE FROM admin_users WHERE email = 'admin@mabrix.com'").run();

const existing = db.prepare('SELECT * FROM admin_users WHERE email = ?').get('Mabricks@gmail.com');
if (!existing) {
  ...
  console.log('Admin user created: Mabricks@gmail.com');
} else {
  console.log('Admin user already exists');
}

// Replace services - remove old ones and add new set
db.prepare("DELETE FROM services WHERE title IN ('E-commerce', 'Digital Strategy')").run();
...
const newServices = [
  ...
  ['Graphics Design', 'graphics-design', 'Visual design solutions that strengthen your brand identity and communicate your message.', '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/></svg>', null, 1, 6, 1],
];

const insert = db.prepare('INSERT OR REPLACE INTO services (title, slug, description, icon, image, featured, active, ordering) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
newServices.forEach(s => insert.run(...s));

// Update social links to match requirements
db.prepare("DELETE FROM social_links WHERE platform IN ('LinkedIn', 'YouTube')").run();
const socialUpdates = [
...
const insertSocial = db.prepare('INSERT OR REPLACE INTO social_links (platform, url, active, ordering) VALUES (?, ?, ?, ?)');
socialUpdates.forEach(s => insertSocial.run(...s));

// Add contact_phone to settings if not present
const phoneSetting = db.prepare('SELECT * FROM site_settings WHERE key = ?').get('contact_phone');
if (!phoneSetting) {
  db.prepare('INSERT INTO site_settings (key, value, type) VALUES (?, ?, ?)').run('contact_phone', '', 'text');
}

// Add service detail content for each service
